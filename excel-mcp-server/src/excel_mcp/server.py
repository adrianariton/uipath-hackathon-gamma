...
async def get_the_news(
    ...
    """
    Get the news about anything. Anything news related should go thru this function.

    PARAMETERS:  
    filepath: Path to Excel file
    sheet_name: Name of worksheet to write to
    news_query: Query string to search for news
    max_news_results: Maximum number of news results to retrieve, default is 5
    start_cell: Cell to start writing to, default is "A1"
  
    """
    try:
        full_path = get_excel_path(filepath)
        latest_news = await get_latest_news(news_query, max_results=max_news_results)
        result = write_data(full_path, sheet_name, latest_news, start_cell)
        return result["message"]
    except (ValidationError, DataError) as e:
        return f"Error: {str(e)}"
    except Exception as e:
